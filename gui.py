...
def find(search):
    ...
    return results                                                  # returns a dictionary with name and url


def select_name(url):                            # if u select a name  # takes arg url
    html = urlopen(url)
    final_soup = BeautifulSoup(html, 'html.parser')
    movies = final_soup.find_all("b")  # finds all the movies person has worked in
    results = {}
    bio_element = final_soup.find("span", class_="see-more inline nobr-only")   # bio of person
    ...
    for movie in movies:           # creating dict with movies and link
        results[movie.text] = "https://www.imdb.com" + re.findall('href="\s*([^"]*)\s*"', str(movie))[0]
    return bio, results


def select_movie(url, movie_name):                   # if u select a movie
    imdb_html = urlopen(url)                                    # loads html for imdb site of movie
    ...
    try:
        ...
        popularity = "-"
    try:
        top_review = imdb_soup.find_all("div", class_="ipc-html-content ipc-html-content--base")[1].text.strip()
    except:
        try:
    ...
    tomato_search_link = "https://www.rottentomatoes.com/search?search="     # standard search link for rotten tomatoes
    metacritic_search_link = "https://www.metacritic.com/search/movie/"      # meta critic
    met = True
    for words in movie_name.split():             # creating url for sites
    ...
    tomato_html = urlopen(tomato_search_link)                # loads html of rotten tomatoes page
    tomato_soup = BeautifulSoup(tomato_html, 'html.parser')
    tomato_titles = tomato_soup.find_all("a", class_="unset", slot="title")
    tomato_links = []
    for title in tomato_titles:         # creates list of search results links
    ...
    for link in tomato_links:
        ...
        tomato_link_soup = BeautifulSoup(tomato_link_html, 'lxml')              # extracts director from rotten tamatoes
        try:
            tomato_director = tomato_link_soup.find("a", attrs={'data-qa': "movie-info-director"}).text
        except:
            tomato_director = "-"

        if tomato_director == director_name:            # if director from rotten tomatoes is same as imdb it is a match
            ...
            audience_score = "-"
            tomato_meter = "-"
    req = Request(metacritic_search_link, headers={'User-Agent': 'Mozilla/5.0'})   # custom headers for meta critic
    ...
    return score, popularity, director_name, audience_score, tomato_meter, meta_score, plot, top_review,


search_bar = Entry(root, width=50, borderwidth=5,)          # creating a search bar
search_bar.insert(0, "search a movie or celebrity")


def select_click(type_title, click_url, name):  # when u click a button
    print(type_title, click_url, name)
    for widgets in root.winfo_children():      # erasing the screen
        widgets.destroy()
    if type_title == "name":                   # for names
        final_result = select_name(click_url)
        print(final_result)
        scr = final_result[1]
        keys = list(scr.keys())
        len_keys = len(keys)
        bio_list = textwrap.wrap(final_result[0], width=125)    # creating labels
        bio = ""
        n = 0
    ...
    elif type_title == "title":       # for titles
        final_result = select_movie(click_url, name)
        imdb_score_label = Label(root, text=f"IMDB score : {final_result[0]}", font=("Arial", 12))         # creating labels
        popularity_label = Label(root, text=f"Popularity : {final_result[1]}", font=("Arial", 12))
        director_button = Label(root, text=f"Director : {final_result[2]}", font=("Arial", 12))
        ...
        director_button.grid(row=3, column=1, pady=10, padx=50)        # putting labels on screen
...
def search_click():                       # clicking the search button
...
photo = PhotoImage(file="VITMDB3.png")     # putting logo
button = Label(root, image=photo)
button.grid(row=1, column=1)
small_label = Label(text="")
search_button = Button(root, text="search", command=search_click)         # search button
